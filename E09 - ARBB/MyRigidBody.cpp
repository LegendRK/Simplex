...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//your code goes here---------------------
	std::vector<vector3> v3BoxCorners;

	// start from top left - get top face
	v3BoxCorners.push_back(m_v3MaxL); // top right front
	v3BoxCorners.push_back(vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z)); // top right back
	v3BoxCorners.push_back(vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z)); // top left front
	v3BoxCorners.push_back(vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z)); // top left back

	// end at bottom right - get bottom face
	v3BoxCorners.push_back(vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z)); // bottom right back 
	v3BoxCorners.push_back(vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z)); // bottom right front
	v3BoxCorners.push_back(vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z)); // bottom left front
	v3BoxCorners.push_back(m_v3MinL); // bottom left back

	// make them global based
	for (uint i = 0; i < 8; i++)
		v3BoxCorners[i] = vector3(m_m4ToWorld * vector4(v3BoxCorners[i], 1.0f));

	m_v3MaxG = m_v3MinG = v3BoxCorners[0];

	// Get the max and min out of the list
	for (uint i = 1; i < 8; i++)
	{
		if (m_v3MaxG.x < v3BoxCorners[i].x) m_v3MaxG.x = v3BoxCorners[i].x;
		else if (m_v3MinG.x > v3BoxCorners[i].x) m_v3MinG.x = v3BoxCorners[i].x;

		if (m_v3MaxG.y < v3BoxCorners[i].y) m_v3MaxG.y = v3BoxCorners[i].y;
		else if (m_v3MinG.y > v3BoxCorners[i].y) m_v3MinG.y = v3BoxCorners[i].y;

		if (m_v3MaxG.z < v3BoxCorners[i].z) m_v3MaxG.z = v3BoxCorners[i].z;
		else if (m_v3MinG.z > v3BoxCorners[i].z) m_v3MinG.z = v3BoxCorners[i].z;
	}
	//----------------------------------------
